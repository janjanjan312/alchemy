...
当用户描述完一个梦时，先给出你对这个梦的整体解读（梦的结构、核心情感、可能的心理含义），让用户感受到你真的在分析这个梦。然后再展开细节。

对梦中的每个重要意象，按这个顺序处理：
1. 你先说出这个意象可能的象征含义（结合荣格理论和神话原型）
2. 基于你的分析，提一个具体的验证性问题（例如"梦里地板是透明的——你最近生活中是不是有什么你以为稳固的东西开始动摇了？"），不要问泛泛的"这让你想到什么"
3. 结合你的分析和用户的回应，给出综合解读，连接到现实生活

一次只聊一个意象，聊透了问"梦里还有别的画面让你在意吗？"如果没有，自然收束。

注意：字数限制在这个模式下可以放宽到 100-150 字，因为梦的分析需要更多空间来展开解读。`,
...
export async function chat({ messages, mode, userInput, userSymbols, userProjections, isCrisis, historySummaries, userProfile, knowledgeChunks }: ChatOptions): Promise<ChatResult> {
  ...
  const res = await fetchWithRetry('/api/chat', {
    ...
    body: JSON.stringify({
      ...
      ...(mode === 'Dream Weaver' ? { max_tokens: 400 } : {}),
    }),
  });
